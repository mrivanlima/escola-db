...
CREATE TABLE IF NOT EXISTS content.activities (
    ...
    description     TEXT,
    activity_type_id SMALLINT NOT NULL, -- FK to content.activity_types
    ...
    activity_data   JSONB NOT NULL DEFAULT '{}'::jsonb, -- Activity-specific config/questions (validated by type)
    thumbnail_url   TEXT,
    ...
    CONSTRAINT fk_activities_type FOREIGN KEY (activity_type_id)
        REFERENCES content.activity_types (activity_type_id),
    ...
    CONSTRAINT ck_activities_data_is_object CHECK (jsonb_typeof(activity_data) = 'object'),
...
CREATE INDEX IF NOT EXISTS idx_activities_type ON content.activities(activity_type_id);
...
-- Activities are global learning content library used by all schools.
...
COMMENT ON COLUMN content.activities.activity_type_id IS 'FK to content.activity_types lookup table';
COMMENT ON COLUMN content.activities.activity_data IS 'JSON: Type-specific data (questions for quiz, video url for video, game config for game, etc.). Structure varies by activity_type_id.';
COMMENT ON COLUMN content.activities.points_reward IS 'Gamification points awarded upon completion';
COMMENT ON COLUMN content.activities.display_order IS 'Order within parent module (lower numbers appear first)';
